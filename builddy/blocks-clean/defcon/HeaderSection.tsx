...
export default function HeaderSection({ content }: HeaderSectionProps) {
  ...
  return (
    <div className="bg-card border-border px-3 py-2">
      <div className="flex flex-col sm:flex-row text-xs gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            className={`flex items-center gap-1 px-2 sm:px-3 py-1 font-mono text-xs defcon-focus ${getButtonStyle(button.icon)}`}>
              ...
              <span className="hidden">{button.text}</span>
              <span className="">{button.shortText}</span>
              <ChevronDown className="" />
            </Button>
          )}
          <div className="flex items-center gap-2 text-xs">
            <span className="text-muted-foreground hidden">STATUS:</span>
