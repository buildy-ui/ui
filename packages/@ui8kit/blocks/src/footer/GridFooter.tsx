...
const gridFooterContentHooks = {
  ...
  columns: createLayoutContentHook({
    header: (content: GridFooterData) => (
      <Stack gap="lg" align="start" className="col-span-full">
        ...
        {content.description && (
          <Text size="md" c="secondary-foreground" className="max-w-md">
            {content.description}
          </Text>
        )}
  ...
  mega: createLayoutContentHook({
    header: (content: GridFooterData) => (
      <Stack gap="xl" align="start" className="col-span-full">
        ...
        <Group gap="md" align="center" justify="between" className="w-full">
          <Group gap="sm" align="center">
            <Icon component="span" size="2xl" lucideIcon={Building2} c="primary" />
            <Stack gap="xs">
              <Title order={2} size="2xl" fw="bold">
                {content.brand}
              </Title>
          ...
          {/* Newsletter Signup */}
          {content.newsletter && (
            <Card p="md" rounded="lg" shadow="sm" className="bg-card border max-w-sm">
              <Stack gap="sm">
        ...
        <Group gap="xl" align="start" className="w-full">
                ...
                <Group gap="md" className="flex-wrap">
                  {content.features.items?.slice(0, 4).map((feature, index) => (
                    <Group key={index} gap="xs" align="center">
                      <Icon component="span" size="xs" lucideIcon={Award} c="primary" />
                      <Text size="sm" c="secondary-foreground">
                        {feature}
                      </Text>
                    </Group>
          ...
          {/* Contact Info */}
          {content.contact && (
            <Stack gap="sm" className="min-w-[200px]">
              <Text size="sm" fw="semibold">
                Contact Info
              </Text>
              ...
              {content.contact.email && (
                <Group gap="xs" align="center">
                  <Icon component="span" size="xs" lucideIcon={Mail} c="secondary-foreground" />
                  <Text
                    size="sm"
                    c="secondary-foreground"
                    className="hover:text-primary transition-colors"
              ...
              {content.contact.phone && (
                <Group gap="xs" align="center">
                  <Icon component="span" size="xs" lucideIcon={Phone} c="secondary-foreground" />
                  <Text
                    size="sm"
                    c="secondary-foreground"
                    className="hover:text-primary transition-colors"
              ...
              {content.contact.address && (
                <Group gap="xs" align="start">
                  <Icon component="span" size="xs" lucideIcon={MapPin} c="secondary-foreground" className="mt-0.5" />
                  <Text size="sm" c="secondary-foreground">
                    {content.contact.address}
                  </Text>
                </Group>
        ...
        <Stack gap="sm">
          {section.links.map((link) => (
            <Group key={link.name} gap="xs" align="center">
              ...
              <Icon component="span" size="xs" lucideIcon={ExternalLink} c="secondary-foreground" className="opacity-50" />
            </Group>
          ))}
        </Stack>
      </Stack>
  ...
  compact: createLayoutContentHook({
    header: (content: GridFooterData) => (
      <Stack gap="md" align="center" ta="center" className="col-span-full">
        <Group gap="sm" align="center">
          <Icon component="span" size="lg" lucideIcon={Building2} c="primary" />
  ...
  newsletter: createLayoutContentHook({
    header: (content: GridFooterData) => (
      <Stack gap="xl" align="center" ta="center" className="col-span-full">
        {/* Newsletter Section */}
        <Card p="xl" rounded="xl" shadow="lg" className="bg-primary/5 border-primary/20 max-w-lg w-full">
          <Stack gap="lg" align="center">
            <Group gap="sm" align="center">
              <Icon component="span" size="xl" lucideIcon={Mail} c="primary" />
              <Title order={2} size="xl" fw="bold">
                {content.newsletter?.title || "Stay Updated"}
              </Title>
            </Group>
            
            <Text size="md" c="secondary-foreground" ta="center">
        ...
        <Stack gap="md" align="center">
          ...
          {content.description && (
            <Text size="sm" c="secondary-foreground" ta="center" className="max-w-md">
              {content.description}
            </Text>
          )}
        </Stack>
  ...
  sitemap: createLayoutContentHook({
    ...
    item: (section: FooterSection, index: number) => (
      <Card p="md" rounded="lg" shadow="sm" className="bg-card border h-full">
        <Stack gap="md">
          <Group gap="sm" align="center">
