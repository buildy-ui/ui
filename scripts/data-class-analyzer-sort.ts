// bun scripts/data-class-analyzer-sort.ts
...
class DataClassAnalyzer {
  ...
  private generateMapping(): void {
    ...
    // Group data-class attributes by their className (utility classes)
    this.mappings.forEach(item => {
      const key = item.className;
      if (!mapping[key]) {
        mapping[key] = [];
      }
      mapping[key].push(item.dataClass);
    });

    // Sort by keys (utility classes) - this will group duplicates together
    const sortedMapping = Object.keys(mapping)
      .sort()
      .reduce((acc, key) => {
        // Sort data-class values within each group
        acc[key] = mapping[key].sort();
        return acc;
    ...
    console.log(`📋 Structure: utility-classes → [data-class-attributes]`);
  }
  ...
  private findDuplicates(): void {
    ...
    if (duplicates.length === 0) {
      ...
      duplicates.forEach((duplicate, index) => {
        ...
        console.log(`   💡 Suggestion: Create base class like ".${duplicate.dataClasses[0].dataClass.split('-')[0]}-common"`);
      });
      
      console.log(`\n💡 Found ${duplicates.length} exact duplicates that could be consolidated`);
      console.log(`📋 Check the JSON file - duplicates will be grouped together alphabetically!`);
    }
  }
